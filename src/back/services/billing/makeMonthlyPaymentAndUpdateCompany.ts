import { ECompanyStatuses, mapCompaniesFromDB } from "../../mappers/comapnies";
...
export const makeMonthlyPaymentAndUpdateCompany = async (
  companyId: number
): Promise<boolean> => {
  ...
  const companyWithRates = mapCompaniesFromDB(companiesWithRatesDB)?.[0];
  ...
  const next = DateTime.fromJSDate(
    new Date(companyWithRates.nextPayment)
  ).toUTC();
  ...
  l.i({
    t: componentName,
    m: `check company #${companyId} for renew`,
    p: companyWithRates,
  });
  ...
  const isNotAvailableForRenew =
    companyWithRates.balance < companyWithRates.perMonth;

  // if we block company
  if (isNotAvailableForRenew) {
    await client.query("UPDATE companies SET status = $1 WHERE id = $2", [
      ECompanyStatuses.UNPAID,
      companyId,
    ]);
  ...
  await client.query(
    ...
    [
      EPaymentStatuses.PAID,
      companyWithRates.perMonth,
      companyId,
      EPaymentTypes.WITHDRAW,
    ]
  );
