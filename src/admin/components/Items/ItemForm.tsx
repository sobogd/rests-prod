...
const ItemFormComponent: FC<Props> = ({
  ...
  const { setValues, resetForm, getFieldProps, setFieldValue } =
    useFormikContext();
  const category = getFieldProps('c').value;

  const langs = useAuth()?.whoami?.company?.langs ?? [];

  const [loadItem, { data, isLoading: isLoadingItem, isFetching, requestId }] =
    useLazyGetItemDetailsQuery();
  ...
  return (
    <Form>
      <ModalRests
        ...
        isShow={
          selectedItemId === null ||
          Number(selectedItemId) > 0 ||
          Number(selectedCopyItemId) > 0
            ? true
            : false
        }
        moreButtons={
    ...
    <Formik
      ...
      onSubmit={(
        ...
        method({
          ...
          ot:
            values?.ot?.map((i) => i.map((i) => ({ ...i, id: undefined }))) ??
            [],
          vt:
            values?.vt?.map((i) => i.map((i) => ({ ...i, id: undefined }))) ??
            [],
          o:
            values?.o?.map((i) => ({ ...i, p: Number(i.p), id: undefined })) ??
            [],
          v:
            values?.v?.map((i) => ({ ...i, p: Number(i.p), id: undefined })) ??
            [],
        }).then((res) => {
          // @ts-expect-error
          if (res.error) {
